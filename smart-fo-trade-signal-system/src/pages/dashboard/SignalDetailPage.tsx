...
const SignalDetailPage = () => {
  ...
  const getCurrentPrice = (signal: Signal | SampleSignal): number => {
    ...
    return (signal as ExtendedSignal).currentPrice ??
      (signal as ExtendedSignal).entry ??
      (signal as any).entryPrice ??
      0;
  };
  ...
  const getSymbol = (signal: Signal | SampleSignal): string => {
    ...
    // For API signals, combine stock and option information or use symbol
    const apiSignal = signal as ExtendedSignal;
    return apiSignal.symbol ||
      ((apiSignal.stock ? apiSignal.stock : '') + (apiSignal.option ? ' ' + apiSignal.option : ''));
  };
  ...
  const getStatus = (signal: Signal | SampleSignal): string => {
    ...
    const apiSignal = signal as ExtendedSignal;
    return apiSignal.status ||
           apiSignal.orderStatus ||
           (apiSignal.executedOrder ? 'EXECUTED' : 'PENDING');
  };
  ...
  const getCreatedAt = (signal: Signal | SampleSignal): string => {
    ...
    const apiSignal = signal as ExtendedSignal;
    if (typeof apiSignal.generatedAt === 'string') return apiSignal.generatedAt;
    if (apiSignal.generatedAt instanceof Date) return apiSignal.generatedAt.toISOString();
    return '';
  };

  // Get entry price
  const getEntryPrice = (signal: Signal | SampleSignal): number => {
    if (isSampleSignal(signal)) return signal.entryPrice;
    return (signal as ExtendedSignal).entryPrice ??
           (signal as ExtendedSignal).entry ??
           0;
  };

  // Get target price
  const getTargetPrice = (signal: Signal | SampleSignal): number => {
    if (isSampleSignal(signal)) return signal.targetPrice;
    return (signal as ExtendedSignal).targetPrice ??
           (signal as ExtendedSignal).target ??
           0;
  };

  // Get stop loss
  const getStopLoss = (signal: Signal | SampleSignal): number => {
    if (isSampleSignal(signal)) return signal.stopLoss;
    return (signal as ExtendedSignal).stopLoss ??
           (signal as ExtendedSignal).stop ??
           0;
  };

  // Get type
  const getType = (signal: Signal | SampleSignal): string => {
    if (isSampleSignal(signal)) return signal.type;
    return (signal as ExtendedSignal).type;
  };

  const entryPrice = getEntryPrice(signal);
  const currentPrice = getCurrentPrice(signal);
  const targetPrice = getTargetPrice(signal);
  const stopLoss = getStopLoss(signal);
  const type = getType(signal);

  const isProfitable = isBuyType(type)
    ? currentPrice > entryPrice
    : currentPrice < entryPrice;

  const priceDifference = Math.abs(currentPrice - entryPrice);
  const percentageDifference = (priceDifference / (entryPrice || 1)) * 100;
  ...
  if (isSampleSignal(signal)) {
    ...
    currentProfitPercentage = (currentProfit / (entryPrice || 1)) * 100;
    potentialProfitPercentage = (potentialProfit / (entryPrice || 1)) * 100;
  }
      ...
      <div className="bg-white shadow overflow-hidden rounded-lg">
        <div className="px-4 py-5 sm:px-6 flex justify-between items-center">
          ...
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
            isBuyType(type) ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
          }`}>
            {isBuyType(type) ? (
        ...
        <div className="border-t border-gray-200">
          <dl>
            <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{formatPrice(entryPrice)}</dd>
            </div>
            ...
            <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{formatPrice(targetPrice)}</dd>
            </div>
            <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt className="text-sm font-medium text-gray-500">Stop Loss</dt>
              <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{formatPrice(stopLoss)}</dd>
            </div>
            {isSampleSignal(signal) && (
              <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
